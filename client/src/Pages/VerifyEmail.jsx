...
import { resendOtp as otpResend, verifyOtp as validateOtp } from "@/Store/AuthSlice";
...
const VerifyEmail = () => {
  ...
  const resendOtp = useCallback(async () => {
    ...
    try {
      ...
      const errorMsg = error || "Failed to resend OTP. Please try again.";
      setError(errorMsg);
      toast.error(errorMsg);
    } finally {
      setTimeout(() => setResendDisabled(false), 30000); // Re-enable button after 30 seconds
    }
  }, [dispatch, resendDisabled, loading]);
  ...
  return (
    <section className="w-full h-screen flex flex-col items-center justify-center bg-neutral-950">
      ...
      >
        ...
        <p className="text-gray-600 mb-2 text-center">Enter the OTP sent to your email.</p>
        <p className="text-gray-500 text-sm italic text-center mb-4">Copy-paste functionality is disabled.</p>
        ...
        <button
          type="submit"
          className={`w-full bg-blue-600 text-white font-bold py-2 px-4 rounded-lg transition hover:bg-blue-700 focus:outline-none ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
          disabled={loading}
        >
